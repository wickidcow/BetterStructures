...
public class FitAnything {
    ...
    public static void commandBasedCreation(
            Chunk chunk,
    ...
    protected void randomizeSchematicContainer(
            Location location,
            GeneratorConfigFields.StructureType structureType) {
        ...
        if (schematicContainer != null) {
            ...
            verticalOffset = schematicContainer.getClipboard().getMinimumPoint().y()
                    - schematicContainer.getClipboard().getOrigin().y();
        }
    }
    ...
    protected void paste(Location location) {
        ...
        // Schematic prepares and tickets every horizontal structure chunk before invoking this
        // callback, so terrain sampling cannot accidentally force neighboring chunks synchronously.
    ...
    /**
     * Runs off the primary thread through the fork's required FAWE provider while the structure's
     * chunks are still ticketed. Pedestal fill, tree cleanup, marker removal, and armor-stand
     * clipboard work no longer land as one large Bukkit completion spike.
     */
    ...
    private Runnable onPasteComplete(FitAnything fitAnything, Location location) {
        return () -> {
            if (DefaultConfig.isNewBuildingWarn()) {
                String structureTypeString = fitAnything.structureType.toString()
                    ...
                    if (player.hasPermission("betterstructures.warn")) {
                        player.spigot().sendMessage(
                                SpigotMessage.commandHoverMessage(
                                        "[BetterStructures] New " + structureTypeString
                                                + " building generated! Click to teleport. Do \"/betterstructures silent\" to stop getting warnings!",
                                        "Click to teleport to " + location.getWorld().getName() + ", "
                                                + location.getBlockX() + ", " + location.getBlockY() + ", "
                                                + location.getBlockZ() + "\n Schem name: "
                                                + schematicContainer.getConfigFilename(),
                                        "/betterstructures teleport " + location.getWorld().getName() + " "
                                                + location.getBlockX() + " " + location.getBlockY() + " "
                                                + location.getBlockZ()));
                    }
                }
    ...
    private void assignPedestalMaterial(Location location) {
        ...
        // Keep the sample count bounded even for very large structures instead of scanning every
        // third block in all three dimensions on the primary thread.
    ...
    private void fillChests() {
        ...
        for (Vector chestPosition : schematicContainer.getChestLocations()) {
            ...
            if (!(chestLocation.getBlock().getState() instanceof Container container)) {
                Logger.warn("Expected a container for " + chestLocation.getBlock().getType()
                        + " but didn't get it. Skipping this loot!");
                continue;
            }
